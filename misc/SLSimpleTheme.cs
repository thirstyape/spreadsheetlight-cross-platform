	...
	internal void CalculateRowColumnInfo()
	{
		...
		if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
		{
			...
			for (i = 0; i < 10; ++i)
			{
				...
				var fWidthG = g.MeasureString(string.Format("_{0}_", i), usablefont).Width - fDoubleUnderscoreWidthG;
			...
			is96dpi = bmGraphics.HorizontalResolution < 108;
		}
		...
		if (is96dpi && (this.MinorLatinFont.Equals("Rockwell Condensed", StringComparison.OrdinalIgnoreCase) || this.MinorLatinFont.Equals("Tw Cen MT Condensed", StringComparison.OrdinalIgnoreCase)))
		...
		// Total guess here, primarily for Linux, splitting the difference between Verdana and Rockwell Condensed in non 96-dpi
		if (iMaxDigitWidth < 2)
			iMaxDigitWidth = 10;
		...
		{
			var fStepInterval = i / (double)(iMaxDigitWidth - 1);
			var fStepSize = Math.Truncate(256.0 * fStepInterval) / 256.0;
		...
		this.fThemeColumnWidth = iWholeNumber + listColumnStepSize[iRemainder];
		this.lThemeColumnWidthInEMU = iDefaultColumnWidthInPixels * SLDocument.PixelToEMU;
